...
def substrings(string)
  ...
  while start < string.length
    start2 = start
    while start2 < string.length
      result << string[start..start2]
      start2 += 1
    end
    start += 1
  end
...
def subwords(word, dictionary)
  # substrings(word).select {|subby| dictionary.include?(subby)}
  subwords = []
  subs.each do |subby|
    if dictionary.include?(subby)
      subwords << subby
    end
  end
  subwords
